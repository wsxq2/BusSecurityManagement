 ...
 *                               main.c                               *
 **********************************************************************/
/* TODO: 查询后再查询会出错: `Commands out of sync; you can't run this command
 * now`  <2018-11-01, wsxq2> */
/* TODO: 从文件导入数据无法确保用户自定义完整性（因为触发器不能用于`LOAD DATA
 * LOCAL INFILE`） <2018-11-01, wsxq2> */
    ...
    for (int i = 0; i < num_fields; i++) {
      if (i == 0) {
        while ((field = mysql_fetch_field(result))) {
          printf("%-20.20s", field->name);
        }

        printf("\n");
      }
      printf("%-20.20s", row[i] ? row[i] : "NULL");
    }
  }
...
char* s[8] = {"RenYuan",  "DuiZhang", "CheDui",     "XianLu",
              "CheLiang", "SiJi",     "LuDuiZhang", "WeiZhang"};
...
int main() {
  ...
  if (mysql_real_connect(con, "localhost", "wsxq", "658231", "bus", 0, NULL,
                         0) == NULL) {
    finish_with_error(con);
  }
  ...
  printf(
      "说明：本程序是公交安全管理系统的前端，主要有 导入数据 和 常用查询 "
      "两个功能，其中导入数据是从 csv 文件中导入。故导入数据时，建议先用 Excel "
      "录入原始数据，再导出为 csv 文件，然后把它们放在 csv "
      "目录下再运行本程序（ csv 目录中放有默认文件用于测试）。按 CTRL+C "
      "退出\n\n");
  signal(SIGINT, stop);
  do {
    ...
    if (memu == 1) {
      printf(
          "请输入数字以导入相应的表（注意按从小到大的顺序），本程序从当前目录下"
          "的 csv/ 目录中导入 csv 文件：\n     0.All 1.RenYuan 2.DuiZhang "
          "3.CheDui 4.XianLu 5.CheLiang 6.SiJi 7.LuDuiZhang 8.WeiZhang\n");
      while (scanf("%d", &insert_data) != 1 ||
             (insert_data < 0 || insert_data > 8)) {
      ...
      printf(
          ...
          "查询某个车队在某个时间段的违章统计信息\n");
      while (scanf("%d", &useful_query) != 1 ||
             (useful_query < 1 || useful_query > 3)) {
      ...
      switch (useful_query) {
        ...
        case 2:
          printf(
              "请输入要查询的司机名、起始时间和结束时间（如`钱智美,2018-01-01 "
              "00:00:00,2018-10-01 00:00:00`）：\n");
          while (scanf("%[^,\n],%[^,\n],%[^,\n]", name, start_time, end_time) !=
                 3) {
            printf(
                "输入错误，请重新输入（如`钱智美,2018-01-01 "
                "00:00:00,2018-10-01 00:00:00`）：\n");
            while ((ch = getchar() != '\n') && ch != EOF) {
            }
          }
        ...
        case 3:
          printf(
              "请输入要查询的车队编号、起始时间和结束时间（如`1,2018-01-01 "
              "00:00:00,2018-10-01 00:00:00`）：\n");
          while (scanf("%[^,\n],%[^,\n],%[^,\n]", cheduibianhao, start_time,
                       end_time) != 3) {
            printf(
                "输入错误，请重新输入（如`1,2018-01-01 00:00:00,2018-10-01 "
                "00:00:00`）：\n");
            while ((ch = getchar() != '\n') && ch != EOF) {
            }
          }
